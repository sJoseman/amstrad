...
;Una vez acaba de cargar el juego mediante basic, salta al codigo del propio juego
;DONDE efectuara 3 lecturas mas al track 40 sector &C6 (WEAK SECTOR)
;para decodificar el programa principal en &5172
...
;se decodificadara en su totalidad XOReando esa zona de memoria en la UPPER ROM
...
;buffer que usa la ROM de BASIC para tokenizar comando de basic tecleado.
...
ld (hl),#64 ;&64 definido en master del disco
...
;lee del track 40 (datos de proteccion anticopia)
...
;decodifica &0170-&028B usando un byte fijo para decodificar y saca el tamano a decodificar de tamano de as.bin
...
;---------- CODIGO DECODIFICADO---------
.L0170 ;zona codificada, ejecuta codigo leido del track 40 para decodificar
...
;CODIGO LEIDO DESDE EL TRACK 40 PROTECCION ANTICOPIA
...
.l961c equ $ + 2
jp #5172 ;Salto real al juego, al igual que la version cinta.

.bucle_lee_weaksector
.l961d ;esto sera borrado al validar proteccion anticopia
...
;tercera lectura a track 40 sector &C6 (weak sector) MISMA LECTURA que primera y segunda
;NO DEVUELVE LOS MISMOS DATOS EN TERCERA LECTURA.

ld hl,l9644 ;direccion de memoria con comando a buscar en roms
            ;&84 ;comando READ SECTOR en la tabla de RSX (&04+&80)
call #bcd4 ;KL FIND COMMAND
;comando encontrado carry=true
;reg c trae numero de ROM que tiene el comando; c=&07 (rom de amsdos)
...
.l9691 ;ES DECIR SE HA VALIDADO QUE EL TRACK 40 ES UN WEAKSECTOR.
ld hl,#be78
