...
;ESTE CODIGO DE CAMBIO DE COLORES NO SE LLAMA EN TERMINATOR 2
;PERO SI SE LLAMA EN HAMMERFIST PARA PONER TINTAS A PANTALLA DE PRESENTACION RECIEN LEIDA
;----CODIGO NO USADO-----------
.pon_colores_otros_juegos
xor a
.la655
push af
ld b,(hl)
...
ld c,b
call #bc38 ;border
;por aqui colores puestos, el cambio no es al momento, se hara cuando salte la siguiente interrupcion
;lo consigue aqui abajo en este jr la676, espera vsync, se producira una interrupcion y se pondran los colores.
ld e,#06
jr la676 ;volvera a rutina original que llamo a cambio de colores (recuerda NO PASA EN T2, SI en hammerfist)
;----FIN CODIGO NO USADO-----------
...
ld hl,#aa00 ;donde leera los datos en RAM del comando READ DELETED DATA (UN NUEVO LOADER)
...
;----codigo no usado loader con terminator 2 PERO SI EN HAMMERFIST-----------------
;ACTUALIZACION, en el juego Hammerfist se llega aqui desde rutina &A687
;JUSTO DESPUES DE ESTE CODIGO ALLI
;ld a,#02 ;tamano datos para comando read deleted data (&02-->512bytes por sector)
;ld (datasize_READDELETEDDATA),a
;add #4a ;mete comando &4C READ DELETED DATA, sumandole el dato de tamano datos
        ;una forma un poco curiosa de ocultar que van a usar este comando del FDC.
;ld (writeCMD_RDData),a
;CALL &A720 ;LLAMA A ESTA RUTINA, REPITO, EN JUEGO HAMMERFIST.
.LA704 ;(&A720 en HAMMERFIST)

ld c,#c8 ;sector
ld hl,#a853 ;direccion donde meter lectura que se va a hacer hacia disco (&A886 EN HAMMERFIST)
ld d,#00 ;track
jp la755 ;EJECUTARA UNA LECTURA DESDE DISCO EN RANGO &A886-&AA85
         ;VOLVERA POR RUTINA QUE LLAMO A ESTA
         ;EN HAMMERFIST EFECTUARA UN CALL &A712, QUE AQUI SERIA UN CALL &A6F6, TINTAS A NEGRO
         ;CAMBIARA REGISTROS DEL CRTC PARA ADECUARLOS A RESOLUCION DE SPECTRUM
         ;CARGA A=&05, HL=&C000 y llamara a rutina similar a la del terminator (&A72A EN HAMMERFIST)
         ;donde consulta en tabla de datos, similar a la del T2
         ;PERO ES MUCHO MAS SIMPLE YA QUE EN HAMMERFIST NO HAY MAS QUE UN SECTOR (&C1) POR TRACK
         ;hasta que llega a valor &05, de=&1b08, b=&3E (TAMANO DATOS A LEER, &3E00)
         ;reg d es track a leer en siguiente lectura de read data, sector sera &c1 decidido a machada
         ;reg e lo usara para calcular un valor, pasa reg c con valor &18 a rutina LA73B
         ;y lo resta decrementando en 1 reg e, &18-&07=&11
         ;NOTA MENOS EL TRACK CERO, LOS DEMAS SOLO TIENEN UN SECTOR IDE &C1
         ;TAMANO DE 8192, AUNQUE EL TAMANO REAL DE ESOS DATOS VARIA!!
         ;y salta aqui abajo, a .la722 (.la755 en hammerfist)
         ;NOTA, LA RUTINA QUE DIGO ES SIMILAR A LA DEL TERMINATOR, NO EXISTE EN ESTE CODIGO COMENTADO.
         ;EJECUTARA UN READ DELETED DATA DE LOS DATOS REQUERIDOS (PANTALLA PRESENTACION LA PRIMERA VEZ).
         ;ESCRIBE EN &C000-&FDFF CORROBORADO &3E00 BYTES, 15.872 BYTES

         ;VOLVERA DESPUES DEL CALL QUE PIDIO ESOS DATOS
         ;HARA UN LD HL,&A86C (DATOS COLORES) Y REALIZARA UN CALL &A654 (pon_colores_otros_juegos)
         ;despues hara
         ;ld a,&06 ;zona a buscar en tabla de datos tamano de datos a leer y track donde leer (sector siempre &C1)
         ;ld hl,&0400 ;direccion de memoria donde meter datos 
         ;call &A72A ;efectua consulta a tabla de datos, reg de=&1D16 reg b=&02 (TAMANO DATOS A LEER &0200)
                     ;y ejecuta otra vez READ DELETED DATA
         ;escribe en &0400-&05FF CORROBORADO, 512BYTES
         ;ld a,&01
         ;ld hl,&0600
         ;call &A72A
         ;datos que genera consulta a tabla de datos reg de=&0101, reg b=&9f (TAMANO DATOS A LEER &9F00)
         ;escribe en &0600-A4FF CORROBORADO &9F00 BYTES 40.704BYTES

         ;despues pone en hl el valor de salto al propio juego, hl=&0641
         ;efectua un borrado de las zonas de memoria &a704-&a87d
         ;efectua un borrado de las zonas de memoria &A600-&A6FB
         ;PONE EN REG BC=&7F8D (GATE ARRAY Mode 1, lower & upper disabled)
         ;PERO NO EJECUTA LA ORDEN AL GATE ARRAY
         ;METE LOS REGISTROS A ESPEJO Y SALTA AL CODIGO DEL JUEGO
         ;NO EJECUTA JAMAS ESE REG BC, POR LO MENOS PARA CAMBIAR EL MODO, PROBE CON PONER MODE 0 Y PONE MODE 1 IGUAL.
         ;INTERRUPCIONES DESHABILITADAS, STACK EN ZONA DE FIRMWARE, LO CAMBIA DENTRO
         ;JP &0641 ;NO SE SI EL JUEGO NECESITA ESO PARA 
         


ld c,a ;DATOS BASURA O DE OTROS JUEGOS?
ld d,#01 ;DATOS BASURA O DE OTROS JUEGOS?
ld e,#01 ;DATOS BASURA O DE OTROS JUEGOS?
ld b,#16 ;DATOS BASURA O DE OTROS JUEGOS?

.la715 ;salta despues de leer un sector entero (rutina LA748 EN HAMMERFIST)
       ;volvera a leer un sector entero hasta que 
       ;HL posicionado en &D100 de primeras (segunda lectura de datos que pedira)
       ;HL posicionado en &E900 de segundas (tercera lectura de datos que pedira)
       ;ESCRIBE LA PANTALLA DE PRESENTACION EN VRAM, AL FINAL VOLVERA DESPUES DEL CALL QUE PIDIO ESTA LECTURA.

ld a,#19
sub e ;reg e se inicializo a &01 antes de saltar aqui
      ;&19-&01, a=&18
cp b ;b=&2D DE PRIMERAS, b=&15 de segundas
jr c,la722 ;si A MENOR que B, activa carry
           ;&19 es menor que 2D de primeras, se cumple el salto, jr c,&A755 en hammerfist.
           ;vuelve a ejecutar un read deleted data a disco, track se incremento a sigiuente
           ;DE SEGUNDAS
           ;&18 es mayor que valor en reg b (&15), con lo cual no hace el salto

;por aqui HL posicionado en &E900 de VRAM (tercera lectura de datos que pedira)
ld a,e ;a=&01
add b ;&01+&15; a=&16
dec a ;a=&15
ld c,a ;c=&15
jp la73b ;jr la76e en hammerfist
         ;retorna a funcion original que llamo al proceso de mandar comando, recibir datos y recibir resultados al comando
         ;&A6C5

.la722 ;LA755 EN HAMMERFIST
       ;LLEGA DESPUES DE LEER DATOS DE CALCULO EN &A886-&AA85
       ;PONER LAS TINTAS A NEGRO
       ;CAMBIAR CRTC A RESOLUCION DE SPECTRUM
ld c,#18
push bc ;&3E18 A PILA
push af ;&11 REG A, A PILA
push hl ;&C000 A PILA (DIRECCION DE CARGA SIGUIENTE FICHERO)
push de ;&1B08, TRACK Y VALOR PARA HACER UN CALCULO
call la73b ;CALL &A76E en hammerfist, pero aqui esta el codigo en LA73B
           ;POR AQUI HA EJECUTADO UN READ DELETED DATA A DISCO
           ;&C000-&D0FF ;4.352BYTES CORROBORADO.
pop de ;de=&1b08
pop hl ;hl=&C000
pop af ;a=&11
pop bc ;bc=&3E18
...
ld b,a ;B=&2D
ld a,h ;a=&C0
add e ;a=&D1 ;siguiente posicion de memoria en ram donde meter datos, byte alto
ld h,a ;hl=&D000, lo dicho, siguiente posicion de memoria donde meter datos
ld e,#01
inc d ;siguitente track a leer
jr la715 ;jr &A748 en hammerfist, aqui esta localizado en &A715

.la73b
;reg c pasado como parametro c=&18
ld a,#4c
ld (writeCMD_RDData),a ;mete comando READ DELETED DATA par mandar al FDC
ld a,#06
ld (datasize_READDELETEDDATA),a ;tamano datos para comando read deleted data (&06-->8.192 bytes por sector)
                          ;EN HAMMERFIST LOS SECTORES TIENEN UN TAMANO DECLARADO DE 8192 BYTES, LO QUE ES CORRECTO AQUI

ld a,c ;a=&18
dec e ;e=&07
sub e ;a=&11
ld (la7dd),a ;cambia el ld de,&0000, por un ld de,&1100 ;ESTA RELACIONADO CON TAMANO REAL DE DATOS EN SECTORES
ld a,e
ld (la7c8),a  ;cambia el ld de,&0000, por un ld de,&0700 ;ESTA RELACIONADO CON TAMANO REAL DE DATOS EN SECTORES
ld e,#c1 ;sector ID inicial a leer
ld c,e ;sector id final a leer ;EN HAMMERFIST ES UN SOLO SECTOR DE 8192BYTES, LOS LEE CON SOLO MANDAR 1 LECTURA A UN SECTOR
jp conf_FDCyREADDELETEDDATA ;la756 ;LA789 EN HAMMERFIST

.la755 ;por aqui se produce salto en HAMMERFIST (LA788 en hammerfist)
ld e,c ;mete sector en reg e
...
ld hl,la839 ;sobreescribe valor de hl, lo sobreescribe con &AA00
            ;LO VA SOBREESCRIBIENDO CON VALOR DE RAM DONDE QUIERE METER LOS DATOS.
jr la7a0
...
.la793 ;la rutina que llama a esta, fue llamada a su vez desde rutina que carga los parametros para ejecutar el comando seek
...
ld bc,&a808 ;lo usa para sobreescribir un jp despues de mandar el comando + parametros
...
;parametros
;de=&A82F comando CMDyPARAM_READ_DELETED_DATA
...
;.la7a3
.manda_CMD_guarda_resultados
;parametros recibidos 1
;bc=&a808 (desde funcion parametros_CMD_READID_mandalo)
;hl=&a84a (donde escribira resultados del comando mandado)
;parametros recibidos 2
;bc=&a820 (desde funcion que manda comando seek al FDC)
;hl NO SE USA EN ESTE SALTO
;parametros recibidos 3
;bc=&A7D1 (salto desde funcion CMDyPARAM_READ_DELETED_DATA)
;hl=&AA00 (donde guardara los bytes leidos con comando READ DELETED DATA, un nuevo loader cargara)


;bc viene como parametro desde la rutina que llama a esta
;trae la direccion a la que se saltara despues de mandar comando, ya que dependiendo del comando, necesita saltar a una u
;otra funcion para actuar segun comando mandado.
...
ld bc,#fb7e ;FDC MAIN STATUS REGISTER
.la7c8 equ $ + 2
ld de,#0000 ;PARA RUTINA &A7D1
            ;EN T2 SIEMPRE ES DE=&0000
            ;PERO EN HAMMERFIST TOMA ESTOS VALORES &0700 Y &1500
            ;ESTA RELACIONADO CON EL FORMATO DEL DISCO, EN T2 NO MIENTE EN EL TAMANO DE SECTOR DECLARADO
            ;PERO EN EL HAMMERFIST EL TAMANO DE SECTOR DECLARADO NO ES EL MISMO QUE LOS DATOS REALES EN EL DISCO.
            ;LA RUTINA HARA LO MISMO, EJECUTARA UN READ DELETED DATA DE LOS DATOS REQUERIDOS AL FDC.

.la7ca equ $ + 1
jp la7d1 ;sobreescribe este jp a la direccion correcta de salto
         ;la primera vez jp &a808 (GUARDA resultados DEVUELTOS POR EL COMANDO MANDADO READID), 
         ;segunda &a820, comando seek
         ;mucho despues &A7D1
;no volvera por aqui, si no a la funcion que llamo a esta subfuncion

.la7cc ;NUNCA SALTA EN T2, SALTARA EN JUEGOS QUE NO PASEN REG DE=&0000 A RUTINA &A7D1
       ;LEERA DE DATA REGISTER, PERO NO HACE NADA CON ESE DATO, VUELVE A MAIN STATUS REGISTER
       ;Y SEGUIRA COMPROBANDO SI EL FDC ESTA LISTO PARA RECIBIR/ENVIAR DATOS.
       ;POR EJEMPLO EN HAMMERFIST SI PASA POR AQUI, ESTA RELACIONADO CON EL TAMANO DECLARADO DE SECTORES
       ;NO ES EL MISMO QUE EL TAMANO DE LOS DATOS REALES QUE HAY EN ESE SECTOR.
inc c
in a,(c)
dec c
dec de

.la7d1 ;viene por aqui despues de ejecutar un READ ID y un comando SEEK para posicionar cabezal
       ;procedera a ejecutar un READ DELETED DATA
       ;PARAMETROS RECIBIDOS
       ;REG HL, DIRECCION DE RAM DONDE ESCRIBIR LOS DATOS QUE SE VAN A PEDIR AL FDC CON READ DELETED DATA
       ;REG DE SI IGUAL A &0000 SE COMPORTA DIFERENTE A SI NO IGUAL A &0000, PERO EL RESULTADO ES EL MISMO.
       ;LA FUNCION HACE EXACTAMENTE LO MISMO
       ;EN TERMINATOR RECIBE SIEMPRE DE=&0000
       ;EN HAMMERFIST RECIBIRA EN ALGUN CASO DE=&0700 (ESCRIBIENDO PANTALLA DE CARGA)
       ;TAMBIEN RECIBIRA DE=&1500 CUANDO ESCRIBA EN &0400 (HAMMERFIST)

in a,(c) ;bc=&FB7E Main status register FDC. 
jp p,la7d1 ;comprueba si el fdc esta listo o no para recibir/enviar datos, si no buclea
ld a,d
or e ;de=&0000 de primeras
jp nz,la7cc ;no se cumple, siempre que reg de, pasado como parametro, sea &0000 JAMAS SALTARA
            ;SOLO EN JUEGOS QUE NO SE PASE &0000 SALTARA

;viene por aqui de primeras
.la7dd equ $ + 2
ld de,#0000 ;por aqui sobreescribe el valor que toma reg de
            ;este valor DEPENDE DEL JUEGO Y DEL FORMATO DEL DISCO
            ;EN TERMINATOR 2 SIEMPRE ES VALOR &0000
            ;EN HAMMERFIST SE PASAN VALORES &1100 1800 &1500 &0200 &1800 &0F00
            ;LA DIFERENCIA ES QUE EL FORMATO DEL T2 NO MIENTE CON EL TAMANO REAL DE LOS SECTORES
            ;EN EL HAMMERFIST EL TAMANO DECLARADO NO ES EL MISMO TAMANO REAL DE DATOS DE LOS SECTORES
            ;POR ESO CAMBIA AQUI ESOS VALORES.
.la7de
.escribe_datos_ram_loader1
inc c ;se posiciona en FDC DATA REGISTER, que es por donde recibe los datos que va leyendo de disco
in a,(c)  ;lee dato
ld (hl),a ;lo guarda en memoria ram &AA00-&BDFF CORROBORADO &1400 BYTES ESCRITOS (5120bytes).
          ;datos REALES de lo cargado &AA00-&ACE2 (lo demas esta rellenado con &E5, no data en disco)
dec c ;bc=&FB7E Main status register FDC.
inc hl ;siguiente posicion de RAM donde leer datos pedidos a disco
dec de ;&FFFF
ld a,d
or e
jp z,la7fb ;comprueba si reg de llega a Zero
           ;evidentemente con un valor de &FFFF en reg de, JAMAS se cumplira esto, se saldra por otra zona de la rutina.
           ;PERO, con valor de de mas bajo (&1500 en hammerfist) si se llega a producir este Z y saltara.

.la7ea
;bc=&FB7E FDC MAIN STATUS REGISTER
in a,(c)
jp p,la7ea ;comprueba si el fdc esta listo o no para recibir/enviar datos, si no buclea
and #20 ;se interesa por bit 5 que indica si el FDC aun esta en execution mode del comando
        ;en READ DATA en execution mode, esta leyendo los datos requeridos desde el disco.

jp nz,escribe_datos_ram_loader1  ;mientras NZ se estan recibiendo datos a cargar desde el FDC!! ;la7de

;por aqui acabo de leer los datos requeridos.
jp la805

.la7f7 ;no se ha acabado el execution mode, salta por aqui
inc c ;se posiciona en Data Register, que en teoria se deberia usar para recibir los RESULT BYTES al comando mandado.
in a,(c) ;lee los en teoria RESULT BYTES, PERO REALMENTE NO SE HA SENALIZADO EL FIN DE EXECUTION MODE
         ;sigue por rutina que consulta de nuevo Main status register FDC
dec c ;MAIN STATUS REGISTER

.la7fb ;jamas se llegara por aqui siendo el parametro de fin de bucle = &FFFF
       ;pero con valores mas bajos (&1500 en hammerfist) SI se llega por esta zona.
       ;ESPERA A QUE MAIN STATUS REGISTER DE EL COMANDO COMO ACABADO IGNORANDO LOS RESULT BYTES DEL DATA REGISTER

in a,(c) ;&f0 la primera vez que pasa por aqui
jp p,la7fb ;NO se cumple el salto
and #20 ;se interesa por bit 5 que indica si el FDC aun esta en execution mode del comando
        ;en READ DATA en execution mode, esta leyendo los datos requeridos desde el disco.
jp nz,la7f7 ;con valor &F0 sigue en execution mode con lo cual NO DA ZERO
            ;BIT 5 EN EL AND ES 1 EN LOS DOS COMPARADORES, CON LO CUAL DA 1, LO QUE ES NOZERO.
            ;BUCLEA IGNORANDO LOS RESULT BYTES DEL DATA REGISTER, Y MIRANDO BIT5 DEL MAIN STATUS REGISTER
            ;HASTA QUE NO SE CUMPLE EL NZ, Y SIGUE POR AQUI ABAJO.


.la805 ;POR AQUI SE LLEGA DE DOS MANERAS, CON REGISTRO DE LLEGANDO A &0000, VIENE JUSTO DE AQUI ATRAS
       ;O AUNQUE NO LLEGUE A &0000
       ;SI EL BIT 5 DEL MAIN STATUS REGISTER SENALIZA EL FIN DE LECTURA DE DATOS, SALTA A AQUI DESDE UN POCO MAS ARRIBA

;SEA COMO SEA LLEGADOS AQUI, SE HA REALIZADO LA ESCRITURA EN RAM DE LOS DATOS PEDIDOS AL FDC.

ld hl,#a84a ;para guardar datos devueltos por el comando READ DELETED DATA que hizo escritura en &AA00-&ACE2
            ;es el loader1, con datos reales en rango &AA00-&ACE2



.la808 ;viene por aqui despues de mandar comando y parametros FUNCION manda_CMD_guarda_resultados MODIFICANDO SU JP FINAL
       ;O DESPUES DEL COMANDO READ DATA TERMINADO EXECUTION MODE. JUSTO EN LA FUNCION COMENTADA AQUI ATRAS
       ;EN LOS DOS CASOS HACE LO MISMO, ESCRIBE LOS RESULT BYTES DEL COMANDO MANDADO.

;reg hl viene cargado con direccion de memoria RAM donde se escribiran los resultados al comando mandado.
;bc=&FB7E FDC MAIN STATUS REGISTER
...
in a,(c)
jp p,la820
ret ;retorna a funcion original que llamo al proceso de mandar comando, recibir datos y recibir resultados al comando
