...
esperando_RQM
  ...
  add a ;aqui mira si el bit 7 de A esta a 1.
        ...
        ;de esta manera el bit 7 de A se mete en el CARRY
        ;y lo que era el bit6 PASA A SER AHORA EL BIT7
        ;esto lo hace para controlar el bit7 y despues el bit 6
        ;por ejemplo &C0 es &11000000 (b7 y b6 seteados)
        ;&c0 + &c0 = 1 10000000 (el carry toma el valor del bit 7)
        ;                       (y el bit 6 es ahora el nuevo bit7)
        ;es decir, hemos SHIFTeado a la izquierda los bits de A

  jr nc,esperando_RQM

  add a ;vuelve a comprobar el bit 7 a 1 (recuerda que antes era el bit 6)
        ;el bit 6 es DIO (DATA INPUT/OUTPUT)
  ...
  jr nc,listo_para_mandar ;transferencia en dirección CPU->FDC
  
  ;por aqui transferencia FDC->CPU, NO podemos mandarle comandos al data register.
  pop af 
  ret

  ;ok, por aqui todo esta listo para el envio del dato al DATA REGISTER.
listo_para_mandar
  pop af
...
;el flag de signo se usa para comprobar que el bit 7 del registro A es un 0 o un 1.
;si el bit 7 del registro A es 1 es NEGATIVO y si es 0 es POSITIVO
;aqui abajo lo usa para saber si el bit7 del MAIN STATUS REGISTER es 1
;no le importa si A es negativo o positivo, solo le importa saber que estado tiene el bit7

bucle_espera_RQM
  ;aqui estamos leyendo del puerto &fb7e que es el Main Status Register del FDC
  in a,(c)
  jp p,bucle_espera_RQM ;esperamos a que el data register tenga preparada la informacion a leer.
                        ;para dilucidarlo usamos el bit 8 del MAIN STATUS REGISTER).
  ...
  ;ya hemos comprobado que el DATA REGISTER esta listo para entregarnos 1 dato.
  ;ahora debemos comprobar que seguimos en EXECUTION-PHASE para ir cogiendo los datos
  ;que nos sirve el DATA REGISTER, cuando ya no estemos en EXECUTION-PHASE es porque
  ;el DATA REGISTER ha acabado ya de servirnos los datos solicitados.
  and #20 ;a=%00100000 ;mira si AUN estamos en EXECUTION-PHASE
          
  
  ret z ;si da Z es porque se acabo la EXECUTION-PHASE, es decir, ya no hay mas datos que leer
